...
REPORTS_BUCKET = 'aws-selenium-test-results'
...
def get_s3_client():
    return boto3.client('s3', config=Config(read_timeout=900, connect_timeout=900, max_pool_connections=10))
...
def lambda_test_list(event, context):
    ...
    print(f"List of cases {test_cases}")
    if event['action'] == 'run_tests':
        ...
        client = boto3.client(
            'lambda',
            region_name=REGION,
            config=Config(read_timeout=900, connect_timeout=900, max_pool_connections=10)
        )

        stats = {'passed': 0, 'failed': 0, 'passed_tc': [], 'failed_tc': []}

        def invoke_test(tc_name):
            print(f'"tc_name": "{tc_name}", "tags": "{tags}"')
